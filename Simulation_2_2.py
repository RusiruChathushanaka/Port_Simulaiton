...
class Prime_Mover(object):
    ...
    def __init__(self,env,JCT,SAGT,CICT,current_terminal="park",next_terminal="park",start_terminal="park",end_terminal="park"):
        ...
        self.action = env.process(self.run_sim(env,JCT,CICT,SAGT))
    ...
    def run_sim(self,env,JCT,CICT,SAGT):

        while True:
            #print("************************",JCT_to_Other.at[0,"Destination Terminal"])
            yield env.timeout(np.random.randint(20,30))
            print(self.id)
            print("hjvjvj")
            ...
            if self.current_terminal == "park":
                ...
                env.process(terminal_loading(self,env))
                # self.terminal_loading(env)

                env.process(moving(self,env))
                # self.moving(env)

                env.process(terminal_unloading(self,env,JCT,CICT,SAGT))
...
def terminal_unloading(self,env,JCT,CICT,SAGT):
...
def main(env):
    ...
    if len(pm_list) == 0:
        for pm in range(60):
            pm = Prime_Mover(env,JCT,SAGT,CICT)
            pm_list.append(pm)
            yield PARKING.put(1)
            print(len(pm_list))
